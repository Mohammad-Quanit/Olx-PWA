let divFav = document.getElementById('favAds-div');
let a = 0;
let offlineAds = false;


firebase.auth().onAuthStateChanged(user => {

  if (user) {
    db.collection('favourites').get().then(snapshot => {
      ...
      snapshot.forEach(change => {
        ...
        div1.addEventListener('click', event => {
          // console.log('yes', change.id)
          showAdDetails(event, change.id)
        })
        ...
        image.src = change.data().favAds.ad_photos[0];
        heading.textContent = change.data().favAds.ad_title;
        time.textContent = change.data().favAds.ad_time;
        price.textContent = 'Rs : ' + change.data().favAds.ad_owner_amnt;
        place.textContent = change.data().favAds.ad_owner_city;
    ...
    signoutmsg.innerHTML = "Sign In To see your Favourite!";
    divFav.appendChild(document.createElement('br'));
    divFav.appendChild(signoutmsg);
  }
});

var JSONParseFav = JSON.parse(localStorage.getItem('favAds'));
setTimeout(function () {
  if (offlineAds == false) {
    alert('offline');
    // console.log(JSONParseFav[0].favAds);
    ...
    for (h = 0; h < JSONParseFav.length; h++) {
      ...
      heading.textContent = JSONParseFav[h].favAds.ad_title;
      time.textContent = JSONParseFav[h].favAds.ad_time;
      price.textContent = 'Rs : ' + JSONParseFav[h].favAds.ad_owner_amnt;
      place.textContent = JSONParseFav[h].favAds.ad_owner_city;
      // message.textContent = 'Send Message';
...
function showAdDetails(event, ad_id) {
  ...
  while (divFav.firstChild) {
    divFav.removeChild(divFav.firstChild);
  }

  db.collection('favourites').doc(ad_id).get().then(docs => {
    if (docs.exists) {
      ...
      let fullSlide =
        `
          ...
          <div class="item active">
            <img src="${docs.data().favAds.ad_photos[0]}" style="width: 100%;" class="col-sm-6 w3-hover-opacity w3-image w3-card-4">
          </div>

          <div class="item">
            <img src="${docs.data().favAds.ad_photos[1]}" style="width: 100%;" class="col-sm-6 w3-hover-opacity w3-image w3-card-4">
          </div>
        
          <div class="item">
            <img src="${docs.data().favAds.ad_photos[2]}" style="width: 100%;" class="col-sm-6 w3-hover-opacity w3-image w3-card-4">
          </div>
      ...
      ad_title.textContent = docs.data().favAds.ad_title.toUpperCase();
      ...
      ad_categorie.textContent = 'Categorie: ' + docs.data().favAds.ad_categorie;
      ...
      ad_desc.textContent = 'Description: ' + docs.data().favAds.ad_description;
      ...
      ad_owner_name.textContent = 'Seller: ' + docs.data().favAds.ad_owner_name;
      ...
      ad_price.textContent = 'Price: ' + docs.data().favAds.ad_owner_amnt + ' Rs.';
      ...
      ad_time.textContent = docs.data().favAds.ad_time;
      ...
      db.collection('favourites')
        ...
        .get().then(respo => {
          respo.forEach(docsi => {
            ...
            favouritesIcon.style = 'font-size:40px; color:orange;';
          });
              ...
              .get().then(respo => {
                ...
                if (adExists == false) {
                  db.collection('favourites').add({
                    ...
                    localStorage.setItem('favAds', JSON.stringify({ one: yes }));
                    favouritesIcon.style = 'font-size:40px; color:orange;';
