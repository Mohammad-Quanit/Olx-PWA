...
db.collection('advertise').onSnapshot(snapshot => {
...
document.getElementById('search-Btn').addEventListener('click', (event) => {
  ...
  // db.collection("advertise").get().then(function (querySnapshot) {
...
function showAdDetails(event, ad_id) {
  ...
  db.collection('advertise').doc(ad_id).get().then(docs => {
    if (docs.exists) {
      ...
      let ad_owner_number = document.createElement('button');
      ...
      ad_categorie.style.padding = '5px';
      ...
      message.setAttribute('class', 'msgBtn col-sm-4 w3-margin-right w3-button w3-border w3-border-lime-green w3-round-large w3-lime w3-hover-green w3-text-white w3-hover-text-white')
      message.setAttribute('id', 'messageBtn');
      message.textContent = 'Send Message';


      ad_owner_number.setAttribute('id', 'number');
      ad_owner_number.setAttribute('class', 'col-sm-4 w3-margin-right w3-button w3-border w3-border-lime-green w3-round-large w3-white w3-text-black');
      ad_owner_number.textContent = docs.data().ad_owner_num;
      ad_owner_number.style.fontSize = '15px';
      ...
      favouritesIcon.setAttribute('class', 'col-sm-6 fa fa-heart');
              ...
              .get().then(respo => {
                ...
                if (adExists == false) {
                  db.collection('favourites').add({
                    ...
                    console.log(reso.id);
                    db.collection('favourites').doc(reso.id).get().then(respo => {
                      favAdsArr.push(respo.data());
                      localStorage.setItem('favAds', JSON.stringify(favAdsArr))
                      alert('agaya')
                      console.log(favAdsArr)
                    })
                    favouritesIcon.style = 'font-size:40px; color:orange; cursor: pointer';
                    alert('Succesfully Added to your favourites');
